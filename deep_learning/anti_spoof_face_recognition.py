...
def predict(img, model):
	...
	img = cv2.imresize(img, (IMG_SIZE,IMG_SIZE)).astype('float32')
	img /= 255
...
def init():
    ...
    for files in os.listdir('dataset'):
        if files == 'train' or files == 'val':
            for file in os.listdir('dataset/'+files):
                if file != '.DS_Store':
                    for file1 in os.listdir('dataset/'+files+'/'+file):
                        if ".jpg" in file1:
                            images.append('dataset/'+files+'/'+file+'/'+file1)
    # print(images)
